 ...
 *
 * @param {string} [dbName='reactflux-persist'] - Optional custom database name.
 * @returns {PersistAdapter} The IndexedDB persistence adapter.
 */
export function indexedDBAdapter(dbName: string = 'reactflux-persist'): PersistAdapter {
  ...
  function getDB(): Promise<IDBDatabase | null> {
    ...
    dbPromise = new Promise((resolve) => {
      try {
        ...
        request.onerror = () => {
          console.warn(`[reactflux] Failed to open IndexedDB "${dbName}"`)
          resolve(null)
        }
      } catch (err) {
        console.warn(`[reactflux] Exception opening IndexedDB "${dbName}":`, err)
        resolve(null)
      }
    })
